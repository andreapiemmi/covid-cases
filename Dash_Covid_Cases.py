    ...
    html.Div([
     ...
     html.Div(children=[
        ...
        html.Div([
            ...
            dcc.Slider(id='Time-Slider',min=numdate[0],max=numdate[-1],value=numdate[0],
...
def country_selection(s,Tot_or_New):
    ...
    fig1.layout.yaxis2.title="$\\Delta Log(1+Cases)$"
...
def Comparison_DT_1stDiff_Log(s):
    ...
    fig_p2 = px.line(np.log(1+dff).diff()[location], render_mode="webgl",).update_traces(yaxis="y2",line_color='slategray')
    ...
    fig.layout.yaxis2.title="1st Diff in log(1+Cases)"
...
def Exp_Smoo(t,co_selected):
    ...
    return fig, ', '+dff.index[0].strftime('%d %b %y')+'-'+dff.index[-1].strftime('%d %b %y')+'  '+str(accuracy)+'%', location


preds = pd.read_csv('https://raw.githubusercontent.com/andreapiemmi/covid-cases/main/Big_Forecasts_ESRW.csv').iloc[
    7:,:].rename(columns={'Unnamed: 0':'Date'}).set_index('Date')
...
def Exp_Smoo_RW(t,co_selected):
    if co_selected is None:
        ...
        location=co_selected['points'][0]['location']
    dff=preds[[*preds.columns[[location in c for c in preds.columns]]]].iloc[t[0]:t[-1],]
